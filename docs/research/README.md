...
| [read-path-neutrality](read-path-neutrality.md) | 2026-09 | Corrected measurements: every read-path variant reshapes top-10s (198-200 of 200) and none moves accuracy; LoCoMo-200 is not retrieval-bound and cannot arbitrate read-path changes |
| [fresh-corpus-blind-spot](fresh-corpus-blind-spot.md) | 2026-08 | Entity IDF and supersedes demotion both measured zero on the pinned 200 set for structural reasons: single-entity queries and a single-pass corpus; superseded by read-path-neutrality: the measurements behind this were invalid (wrong JSON key) |
| [ablation-baseline](ablation-baseline.md) | 2026-08 | First graph-on vs graph-off measurement on one corpus: identical accuracy (0.690 both), 11 helped / 11 hurt, p=1.0; the graph's query-time contribution is zero and adversarial questions lose 6-to-1 |
